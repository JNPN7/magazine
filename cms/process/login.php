						...
						if($user_info[0]->password == $data['password']){
							if($user_info[0]->role == 'Admin'){
								if($user_info[0]->status == 'Active'){
									...
									redirect('../login.php.php','error','Your account is not active.');
								}
							}else{
								redirect('../login.php','error','You cannot logged in here.');
								// debugger($user_info);
							}
						}else{
							redirect('../login.php','error',"Password doesn't match, Please Retry");
						}
					}else{
						redirect('../login.php','error','Email is not found. Please Register');
					}
				}else{
					redirect('../login.php','error','Password is required');
				}
			}else{
				redirect('../login.php','error','Invalid Email');
			}
		}else{
			redirect('../login.php','error','Email is required.');	
		}
	}else{
